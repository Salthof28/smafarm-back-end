...
export class LivestocksController {
  ...
  @Get()
  async getAllLivestock(@Query('name') name?: string, @Query('location') location?: string, @Query('category_id') category_id?: number, @Query('low_price') low_price?: number, @Query('high_price') high_price?: number): Promise<Livestock[]> {
    try {
  ...
  @UseGuards(JwtAuthGuard)
  @Patch(':id')
  async updatedLivestock(@Request() request, @Param('id', ParseIntPipe) id: number, @Body() body: UpdateLivestockDto): Promise<Livestock> {
    try {
      const user_id = request.user.id;
      const upLivestock: Livestock = await this.livestocksService.updatedLivestock({
        id,
        user_id,
        body
      });
